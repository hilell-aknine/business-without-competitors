...
- **Next planned task:** Visual QA after Vercel auto-deploys. Then deferred items: (a) unify CSS design systems (`css/shared.css` ↔ `assets/css/design-tokens.css`), (b) unified progress dashboard across portal/quiz/practice, (c) breadcrumbs on detail pages, (d) clean dead `.v1-header`/`.v1-hd__*` inline styles from `index.html`.
